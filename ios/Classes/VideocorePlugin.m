#import "VideocorePlugin.h"
#import <AVFoundation/AVFoundation.h>

#if __has_include(<videocore/videocore-Swift.h>)
#import <videocore/videocore-Swift.h>
#else
// Support project import fallback if the generated compatibility header
// is not copied when this plugin is created as a library.
// https://forums.swift.org/t/swift-static-libraries-dont-copy-generated-objective-c-header/19816
#import "videocore-Swift.h"
#endif
...
@interface FLTVideocore : NSObject<FlutterTexture, FlutterStreamHandler>
...
@implementation FLTVideocorePlugin
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  FlutterMethodChannel* channel =
      [FlutterMethodChannel methodChannelWithName:@"videocore"
                                  binaryMessenger:[registrar messenger]];
  FLTVideocorePlugin* instance = [[FLTVideocorePlugin alloc] initWithRegistrar:registrar];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"init" isEqualToString:call.method]) {
    ...
    if (dataSource) {
      ...
      player = [[FLTVideocore alloc] initWithAsset:assetPath frameUpdater:frameUpdater];
    } else {
      dataSource = argsMap[@"uri"];
      player = [[FLTVideocore alloc] initWithURL:[NSURL URLWithString:dataSource]
                                      frameUpdater:frameUpdater];
    }
    int64_t textureId = [_registry registerTexture:player];
    frameUpdater.textureId = textureId;
    FlutterEventChannel* eventChannel = [FlutterEventChannel
        eventChannelWithName:[NSString stringWithFormat:@"videocore/videoEvents%lld",
                                                        textureId]
    ...
    FLTVideocore* player = _players[@(textureId)];
